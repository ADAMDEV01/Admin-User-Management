...
function App() {
  ...
  const navigate = (section: string) => { setActiveSection(section); setSidebarOpen(false) }
  const perPage = 8
  ...
  return <div className="app" onClick={() => menu && setMenu(null)}>
    ...
    <aside className={`sidebar${sidebarOpen ? ' open' : ''}`} onClick={e => e.stopPropagation()}>
      ...
      <nav><p className="nav-label">Workspace</p><Nav icon={<LayoutDashboard/>} label="Overview" active={activeSection === 'Overview'} onNavigate={() => navigate('Overview')}/><Nav icon={<BarChart3/>} label="Analytics" active={activeSection === 'Analytics'} onNavigate={() => navigate('Analytics')}/><Nav icon={<Users/>} label="Users" active={activeSection === 'Users'} badge={stats.total} onNavigate={() => navigate('Users')}/><Nav icon={<FileText/>} label="Content" active={activeSection === 'Content'} onNavigate={() => navigate('Content')}/><p className="nav-label second">Manage</p><Nav icon={<ShieldCheck/>} label="Roles & permissions" active={activeSection === 'Roles & permissions'} onNavigate={() => navigate('Roles & permissions')}/><Nav icon={<Settings/>} label="Settings" active={activeSection === 'Settings'} onNavigate={() => navigate('Settings')}/></nav>
    ...
    <main>
      <header><div className="breadcrumb"><button className="mobile-menu icon-btn" aria-label="Open navigation" onClick={() => setSidebarOpen(true)}><Menu/></button><span>Workspace</span><ChevronRight size={14}/><b>{activeSection}</b></div><div className="header-actions"><button className="icon-btn" aria-label="Help"><HelpCircle/></button><button className="icon-btn" aria-label="Notifications"><Bell/><i/></button><div className="header-avatar">OR</div></div></header>
      {activeSection === 'Users' ? <section className="content"><div className="title-row"><div><p className="eyebrow">Workspace management</p><h1>Users</h1><p className="subtitle">Manage who has access to your workspace and what they can do.</p></div><button className="primary" onClick={() => {setEditing(null); setModal('add')}}><Plus size={18}/> Invite user</button></div>
      ...
      </section> : <section className="content"><div className="placeholder-page"><p className="eyebrow">Workspace</p><h1>{activeSection}</h1><p className="subtitle">This workspace area is ready for your {activeSection.toLowerCase()} tools and settings.</p><div className="placeholder-card"><Check size={20}/><div><h2>{activeSection} is available</h2><p>Use the Users section to manage access, or continue building this workspace area.</p></div></div></div></section>}
    </main>
